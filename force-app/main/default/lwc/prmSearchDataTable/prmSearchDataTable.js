...
export default class datatableSearch extends LightningElement {
    ...
    //Search all data for filter string
    ...
    search(event){
        // disable search if user not typing
        if(!this.ensureFilterableFields()) return;
        // Apply search throttling (no search if still typing)
    ...
    filterData(searchFilter){
        let results; 
        try {
            //search for  exact character match, not case sensitive
