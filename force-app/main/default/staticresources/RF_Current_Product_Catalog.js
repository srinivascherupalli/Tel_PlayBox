...
30.     Ramcharan            08-Nov-2021       21.15               INC000097601288  <Cloudsense><Solution Console> Added logic to Gross margin(%) is required instead of Total cost   
...
console.log('::::::: Current Product Catalog controller resource ::::::');
...
if (CS.SM.registerPlugin) {
    window.document.addEventListener("SolutionConsoleReady", async () => {
        await CS.SM.registerPlugin(AN_COMPONENTS.solution).then(async (SCPlugin) => {
            SC_updatePlugin(SCPlugin);
        });
    });
}


const SC_updatePlugin = async (SCPlugin) => {
    ...
    window.document.addEventListener("SolutionSetActive", async (e) => {
        ...
        await currentBasket.performRemoteAction("SolutionActionHelper", inputMap).then(async (result) => {
            ...
            console.log('GetOppty: ', oppty);
            SRVList = oppty.Price_List__c;
            console.log('SRVList: ', SRVList);
            opportunityStage = oppty.StageName;
            console.log('Stage Name', opportunityStage);
            ...
            console.log('partnerAccount ', partnerAccount);
            if (partnerAccount != null && partnerAccount != '' && partnerAccount != undefined) {
                ...
                console.log('partnerAccountName>> ', partnerAccountName);
                console.log('partnerAccountId>>>>> ', partnerAccountId);
            }
            console.log('OptyType ', OptyType);
            if (opportunityStage != null && opportunityStage != '' && opportunityStage != undefined &&
                (opportunityStage === 'Closed Lost' || opportunityStage === 'Closed Won')) {
    ...
    //SCPlugin.beforeConfigurationAdd = async (componentName, guid)
    SCPlugin.afterConfigurationAdd = async (componentName, guid) => {
        ...
        if (solution.name.includes("Standard Product Solution")) {
            ...
            if (stdProdComp && stdProdComp.schema && stdProdComp.schema.configurations && Object.values(stdProdComp.schema.configurations).length > 0) {
                ...
                Object.values(stdProdComp.schema.configurations).forEach(async (devConfigToUpdate) => {
                    ...
                    console.log(accountIdAttribute);
                    ...
                    console.log('InstallationAddress' + devConfigToUpdate.getAttribute("InstallationAddress").showInUi);
                    if (devConfigToUpdate.getAttribute("ShowSiteLookupAttribute").value) {
                        ...
                        if (installationAddressAttribute != null && installationAddressAttribute != undefined &&
                            ...
                            console.log('coming here' + siteAttribute);
                            updateMap1.set(devConfigToUpdate.guid, [{
        ...
        configs = stdProdComp.updateConfigurationAttribute(configs.guid, attrList1)
            .then((configs) => {
    ...
    //Warning Messages for Revenue Fields
    SCPlugin.afterAttributeUpdated = async function(component, configuration, attribute, oldValueMap) {
        let solution = await CS.SM.getActiveSolution();
        let stdProdComp = solution.getComponentBySchemaName(AN_COMPONENTS.StandardProduct);
        let config = Object.values(solution.getConfigurations())[0];
        let doUpdate = false;
        ...
        let updateTotalCostMap = new Map();
        ...
        if (solution.name === 'Standard Product Solution') {
            if (stdProdComp.schema && stdProdComp.schema.configurations && Object.values(stdProdComp.schema.configurations).length > 0) {
                Object.values(stdProdComp.schema.configurations).forEach(async (devConfigToUpdate) => {
                    ...
                    //P2OB-11731 started
                    //let patt1 = devConfigToUpdate.getAttribute("product_sc");
                    let productName = configuration.attributes.product_sc.displayValue;
                    console.log(productName);
                    ...
                    if (OptyType === "Renewal") {
                        if (devConfigToUpdate.attributes.renewalrevenue.value == '' && devConfigToUpdate.attributes.srm.value >= 0) {
                            if (qtychaged === true || prodUpdate === true) {
                                //if (devConfigToUpdate.attributes.srm.value >= 0 && (devConfigToUpdate.attributes.renewalrevenue.value == '' || ProdQtyUpdate === true)) {
                    ...
                    //P2OB-11769 <Cloudsense><Solution Console> Business Rules for Renewal Revenue End
                    //P2OB-11772 <Cloudsense><Solution Console> Business Rules for Recurring Charge (RC) Start
                    //If OptyType =  [if SRMValue >= 0] and ]current Renewal Revenue = ''] 
					...
					//Updated by Aman Soni for P2OB-12565 to make GrossMargin_Pct mandatory for Purple Products in place of Total Cost attribute || Start
					...
					//Updated by Aman Soni for P2OB-12565 to make GrossMargin_Pct mandatory for Purple Products in place of Total Cost attribute || End
                    ...
                    //let patnerAttributeName = devConfigToUpdate.getAttribute("patnername");
                    ...
                    console.log('updatePartnerChannelValueMap ', updatePartnerChannelValueMap);
                    ...
                    //if (devConfigToUpdate.attributes.family_sc.value != "" && devConfigToUpdate.attributes.domain_sc.value == "" && oldValue != newValue && oldValue != "" && doUpdate === true) {
                    if (devConfigToUpdate.attributes.domain_sc.value == "" && oldValue != newValue && oldValue != "" && doUpdate === true) {
                        /*if (devConfigToUpdate.attributes.family_sc.value != "") {
                             let familyAttribute = devConfigToUpdate.getAttribute("family_sc").name;
                    ...
                    //Commenting start for 13246 defect
                    /*else if (devConfigToUpdate.attributes.product_sc.value != "" && oldValue != newValue && oldValue != "" && doUpdate === true) {
                        let productAttribute = devConfigToUpdate.getAttribute("product_sc").name;
                        resetLookupMap.set(devConfigToUpdate.guid, [{
                                name: productAttribute,
                                value: '',
                    ...
                    /*else if (devConfigToUpdate.attributes.product_sc.value != "" && devConfigToUpdate.attributes.domain_sc.value === "" && doUpdate === true) {
                        let productAttribute = devConfigToUpdate.getAttribute("product_sc").name;
                        //let familyAttribute = devConfigToUpdate.getAttribute("family_sc").name;
                        resetLookupMap.set(devConfigToUpdate.guid, [{
                                name: productAttribute,
                    ...
                    //Commenting End for 13246 defect

                    /*else if (devConfigToUpdate.attributes.family_sc.value != "" && oldValue != newValue && oldValue != "" && devConfigToUpdate.attributes.domain_sc.value === "") {
                        let familyAttribute = devConfigToUpdate.getAttribute("family_sc").name;
                        resetLookupMap.set(devConfigToUpdate.guid, [{
                                name: familyAttribute,
                                value: '',
                    ...
                    if (devConfigToUpdate.attributes.oc.value != "") {
                        ...
                        /*var parts = Number(ocattributeval).toFixed(2).split(".");
                    ...
                    if (GMVisattributeval != 0 || GMVisattributeval != 0.00) {
                        ...
                        if (revaddval != 0 || revaddval != 0.00) {
                            ...
                            //let GMpctattributeval = devConfigToUpdate.attributes.grossmargin_pct.value;
                   ...
                   /* Changes for P2OB-12535 start
                    ...
                    if (site != null && site != undefined && site.value != null && site.value != undefined && site.value != '') {
                        ...
                        console.log('JSONNode>>' + JSON.stringify(JSONNode));
                    ...
                    var DomainNode = {};
                    if (productLookupValue != null && productLookupValue != undefined && productLookupValue.value != null && productLookupValue.value != undefined 
                        && productLookupValue.value != '' && isProductChanged) {
                        ...
                        console.log('DomainNode>>' + JSON.stringify(DomainNode));
    ...
    SCPlugin.afterSave = async function(result, configurationsProcessed, saveOnlyAttachment, configurationGuids) {
