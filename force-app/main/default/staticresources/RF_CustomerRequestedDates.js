/*----------------------------------------------------------
  ...
  4. Arinjay S			15.10.2020      EDGE-184551
 6. Laxmi              07.11.2020      added date formation 
 7. Jamil				09.11.2021		Added As part of R34 Fix.
  ------------------------------------------------------------**/
...
window.onInitOESchema["Customer requested Dates"] = async function (guid) {
	...
	//EDGE-165632  Automate CRD Date for mobiles on Order Enrichment screen for Corporate Mobile Plus
	/////////////////
	...
	let updateMapNew = {};
	updateMapNew[guid] = [];
	//let configID = await OE.getConfigGuidForOEGUID(guid);
	...
	if (configName === "Device" ) {	 // Device Added in DIGI-22085
	  if (nbCRD === "0" || nbCRD === "" || nbCRD === 0 || nbCRD.length === 0 || nbCRD===Utils.formatDate(new Date().setHours(0, 0, 0, 0)) ) {
	...
	if (configName === "Mobile Subscription" || configName === "NextGenMobileDevice" || configName === "Enterprise Mobility") {
		//Added NGEM by laxmi
		if (nbCRD === "0" || nbCRD === "" || nbCRD === 0 || nbCRD.length === 0) {
		...
		if  ((  configName === "Enterprise Mobility"  ||  configName === "NextGenMobileDevice"  )
			&& (nbCRD === "0" || nbCRD === "" || nbCRD === 0 || nbCRD.length === 0) 
			&&  (pfCRD === "0" || pfCRD === "" || pfCRD === 0 || pfCRD.length === 0) ) 
		{
	...
	let today = Utils.formatDate((new Date()).setHours(0,0,0,0));
	var Rules = [
		{
			Id: 0,
			SfId: 1929,
			...
			IfTrue: function () {
				Utils.updateOEPayload(
					[
						{ name: "Not Before CRD", readOnly: true, required: false },
						{ name: "Preferred CRD", readOnly: true, required: false }
					],
					guid
				);
		...
		},
		{
			...
			IfTrue: async function () {
				let nbCRD = await Utils.getAttributeValue("Not Before CRD", guid);
				Utils.updateOEPayload([{ name: "Preferred CRD", showInUi: true, readOnly: false, value: Utils.formatDate(nbCRD), displayValue: Utils.formatDate(nbCRD) }], guid);
				return Promise.resolve(true);
			},
			Else: function () {
				return Promise.resolve(true);
		...
		},
		{
			...
			Predicates: [
				...
				},
				{
					...
					operator: "<",
					attValue: today, // $Today //Changed by Venkat from Time.now() to resolve the issue with seconds for the same date.
					group: "0",
					groupConjuction: null,
					conjuction: null
				}
...
window.noPredicate["Customer requested Dates"] = async function (guid) {
	...
	}*/ // Commented As Part R34 OE Fix.
	let component = await solution.getComponentBySchemaName(parentConfig); //Jamil: Added as part of R34.
