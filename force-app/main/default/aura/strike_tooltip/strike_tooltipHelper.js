    ...
    calculateTooltipPosition: function(component) {
        ...
        if (placement == 'right') {
            ...
            if ((containerLeft + tooltipXPos) < 4) {
                ...
                var adjustment1 = (containerRight + Math.abs(tooltipXPos)) - (window.innerWidth - 4);

                tooltipXPos -= adjustment1;
                tooltipStyle.innerHTML = '.st-popover_container .slds-nubbin--top:before { transform: translateX(' + adjustment1 + 'px) rotate(45deg); } .st-popover_container .slds-nubbin--top:after { transform: translateX(' + adjustment1 + 'px) rotate(45deg); }';
            }
            ...
            if ((containerLeft + tooltipXPos) < 4) {
                var adjustment2 = Math.abs(containerLeft + tooltipXPos) + 4;

                tooltipXPos += adjustment2;
                tooltipStyle.innerHTML = '.st-popover_container .slds-nubbin--bottom:before { transform: translateX(' + -adjustment2 + 'px) rotate(45deg); } .st-popover_container .slds-nubbin--bottom:after { transform: translateX(' + -adjustment2 + 'px) rotate(45deg); }';
            } else if ((containerRight + Math.abs(tooltipXPos)) > (window.innerWidth - 4)) {
                var adjustment3 = (containerRight + Math.abs(tooltipXPos)) - (window.innerWidth - 4);

                tooltipXPos -= adjustment3;
                tooltipStyle.innerHTML = '.st-popover_container .slds-nubbin--bottom:before { transform: translateX(' + adjustment3 + 'px) rotate(45deg); } .st-popover_container .slds-nubbin--bottom:after { transform: translateX(' + adjustment3 + 'px) rotate(45deg); }';
            }
        }
